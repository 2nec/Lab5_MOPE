...
def main(m, n):
    ...
    if g_p < g_t:
        print(f"Дисперія однорідна: Gp = {g_p:.5} < Gt = {g_t}")
    else:
    ...
    if n == d:
        n = 8 if n == 4 else 15
        print(f"n=d\nПроводимо експеримент знову з n = {n}")
        return main(m=m + 1, n=n)
    if n == 15:
        print(
            f"Нормоване спрощене рівняння регресії: "
    ...
    if f_p > get_value(f_t, f_3)[f_4]:
        ...
        print(
            ...
            f"Математична модель не адекватна\n"
            f"Проводимо експеримент знову з m = m + 1 = {m + 1} та n = {n}")
        return main(m=m + 1, n=n)
        global count
        count += 1
    else:
        print(
            f"fP = {f_p} < fT = {get_value(f_t, f_3)[f_4]}.\n"
            f"Математична модель адекватна\n")

count = 0
for i in range(500):
    main(m=3, n=4) #Якщо починати експеримент з n = 4, лише у декількох випадках модель буде не адекватна, тому спочатку n було 15, а перехій існував(строка 317)
print(count)
